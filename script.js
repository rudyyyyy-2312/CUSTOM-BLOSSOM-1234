        ...
        const THEMES = [
            ...
            { id: 'cars', name: 'Cars', icon: 'images/theme_cars.png', colors: ['#2B2D42', '#8D99AE', '#EDF2F4'], tag: 'Speed & Style', mood: 'Sleek & Dynamic' },
        ...
        const SCROLL_THEMES = [
            ...
            { id: 'marvel', name: 'Marvel', emoji: 'images/theme_marvel.png', img: 'images/theme_marvel.png', tag: 'Sacred Geometry', desc: 'Find your centre. Intricate patterns that radiate calm, balance and sacred beauty.', accent: '#C9B8E8', accent2: '#F8C8DC', bg: 'linear-gradient(135deg,#F3E8FF,#FFF0F5)', designs: [{ name: 'Violet Lotus', color: '#C9B8E8', icon: '/images/icon_designs.png' }, { name: 'Rose Mandala', color: '#F8C8DC', icon: '/images/icon_designs.png' }, { name: 'Golden Om', color: '#F0D98A', icon: '/images/icon_designs.png' }] },
        ...
        function ProMode({ product, selectedModel, design, onDesignChange, onNext, onBack }) {
            ...
            const tabs = [
                ...
                { id: 'text', label: 'Typography', icon: '✍️' },
            ];
            ...
            return (
                <div className="page fade-in page-enter">
                    ...
                    <div className="section-header" style={{ paddingBottom: 20 }}>
                        <h2 style={{ fontSize: '2.5rem' }}>Design <em>Studio</em></h2>
                        <p>Advanced customizer engine powered by Blossom™</p>
                    </div>

                    <div className="pro-layout-v2" style={{
                        display: 'grid',
                        gridTemplateColumns: 'clamp(300px, 38vw, 480px) 1fr',
                        ...
                        {/* LEFT SIDEBAR: Professional Control Center */}
                        <div className="pro-sidebar" style={{
                            height: '100%', borderRight: '1px solid rgba(0,0,0,0.06)',
                            display: 'flex', flexDirection: 'column', background: '#fff', overflow: 'hidden'
                        }}>
                            {/* Skinit-Style Header */}
                            <div style={{ padding: '32px 40px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                                <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                                    <div style={{ padding: '4px 12px', background: '#e0f2fe', color: '#0369a1', borderRadius: 6, fontSize: 10, fontWeight: 900, letterSpacing: '0.08em' }}>3M™ CAST VINYL</div>
                                    <div style={{ padding: '4px 12px', background: '#fef3c7', color: '#92400e', borderRadius: 6, fontSize: 10, fontWeight: 900, letterSpacing: '0.08em' }}>AIR-RELEASE</div>
                                </div>
                                <h2 style={{ fontSize: '1.8rem', fontWeight: 900, color: '#1a1a1b', fontFamily: 'Outfit, sans-serif', letterSpacing: '-0.02em', lineHeight: 1.1 }}>
                                    Custom Blossom <span style={{ color: 'var(--pink-deep)' }}>Studio</span>
                                </h2>
                                <p style={{ fontSize: 13, color: 'var(--text-mid)', marginTop: 8, fontWeight: 500 }}>
                                    Configuring: {selectedModel ? selectedModel.name : (prod?.name + ' Master V4')}
                                </p>
                            </div>

                            <div className="pro-sidebar-scroll" style={{ flex: 1, overflowY: 'auto', padding: '32px 40px' }}>
                                <div className="pro-nav-tabs" style={{
                                    display: 'flex', gap: 6, marginBottom: 32,
                                    background: '#f1f5f9', padding: 6, borderRadius: 16
                                }}>
                                    {['color', 'pattern', 'effect', 'text'].map(s => (
                                        <button key={s}
                                            onClick={() => setStep(s)}
                                            style={{
                                                flex: 1, padding: '12px 0', borderRadius: 12, border: 'none',
                                                background: step === s ? '#fff' : 'transparent',
                                                color: step === s ? '#1e293b' : '#64748b',
                                                fontSize: 11, fontWeight: 800, cursor: 'pointer',
                                                boxShadow: step === s ? '0 4px 12px rgba(0,0,0,0.05)' : 'none',
                            ...
                            <div style={{ padding: '24px 40px', borderTop: '1px solid rgba(0,0,0,0.04)', display: 'flex', gap: 12 }}>
                                <button className="btn btn-primary" style={{ flex: 1, padding: '16px' }} onClick={onNext}>Preview Render →</button>
                                <button className="btn btn-secondary" style={{ width: 60, padding: 0 }} onClick={onBack}>✕</button>
                            </div>
                        </div>

                        {/* MAIN CONTENT Area: Large Studio Preview */}
                        <div className="pro-main" style={{
                            background: '#f8fafc', position: 'relative',
                            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                            padding: 60, overflowY: 'auto', overflowX: 'hidden'
                        }}>
                            {/* Studio Lighting Effects */}
                            <div style={{ position: 'absolute', top: '10%', left: '10%', width: '40%', height: '40%', background: '#fff', filter: 'blur(100px)', opacity: 0.5, borderRadius: '50%' }} />
                            <div style={{ position: 'absolute', bottom: '10%', right: '10%', width: '30%', height: '30%', background: 'var(--pink-mid)', filter: 'blur(120px)', opacity: 0.1, borderRadius: '50%' }} />

                            <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                                <div className="float-pro">
                                    <ProductPreview product={product} selectedModel={selectedModel} design={design} size={360} rotationY={rotation} />
                                </div>

                                {/* Studio Rotation Control */}
                                <div style={{ marginTop: 40, width: '100%', maxWidth: 320, margin: '40px auto 0', textAlign: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 15 }}>
                                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: isAutoSpinning ? 'var(--pink-deep)' : '#e2e8f0', animation: isAutoSpinning ? 'pulseSoft 2s infinite' : 'none' }} />
                                        <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.2em', color: '#94a3b8', textTransform: 'uppercase' }}>Studio perspective: {rotation}°</span>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                                        <input
                                            ...
                                            style={{ flex: 1, accentColor: 'var(--pink-deep)', cursor: 'ew-resize', height: 4, borderRadius: 2 }}
                                        />
                                        <button
                                            ...
                                            style={{
                                                ...
                                                borderRadius: 12, padding: '6px 14px', fontSize: 9, fontWeight: 900, cursor: 'pointer',
                                                letterSpacing: '0.05em', transition: 'all 0.2s',
                                                boxShadow: isAutoSpinning ? '0 4px 12px rgba(212, 104, 142, 0.25)' : 'none'
                                            }}>
                                            {isAutoSpinning ? 'STOP' : 'AUTO 360'}
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div style={{ marginTop: 40, display: 'flex', gap: 40, justifyContent: 'center' }}>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: 11, fontWeight: 900, color: '#1e293b' }}>ANTI-SCRATCH</div>
                                        <div style={{ fontSize: 9, color: '#94a3b8', marginTop: 4 }}>Protective Lamination</div>
                                    </div>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: 11, fontWeight: 900, color: '#1e293b' }}>ULTRA-SLIM</div>
                                        <div style={{ fontSize: 9, color: '#94a3b8', marginTop: 4 }}>0.23mm Thickness</div>
                                    </div>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: 11, fontWeight: 900, color: '#1e293b' }}>PRECISION FIT</div>
                                        <div style={{ fontSize: 9, color: '#94a3b8', marginTop: 4 }}>Laser-Cut Technology</div>
                                    </div>
                                </div>
                            </div>

                            {/* Bottom Tool Bar */}
                            <div style={{ marginTop: 60, display: 'flex', gap: 15, background: '#fff', padding: '10px 24px', borderRadius: 50, border: '1px solid #e2e8f0', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
                                <button onClick={() => onDesignChange({ ...design, text: '' })} style={{ background: 'none', border: 'none', fontSize: 11, fontWeight: 800, color: '#64748b', cursor: 'pointer' }}>RESET TEXT</button>
                                <div style={{ width: 1, height: 20, background: '#e2e8f0' }} />
                                <button onClick={() => {
                                    const randomColor = COLORS[Math.floor(Math.random() * COLORS.length)];
                                    onDesignChange({ ...design, bgColor: randomColor, pattern: PATTERNS[Math.floor(Math.random() * PATTERNS.length)].id });
                                }} style={{ background: 'none', border: 'none', fontSize: 11, fontWeight: 800, color: 'var(--pink-deep)', cursor: 'pointer' }}>SHUFFLE STYLE</button>
                            </div>
                        </div>
                    </div>
                </div>
